...
// regression case that motivated using matrix-valued and orbital-endpoint-aware
// symmetrization rules globally.
struct ThreebandD4 {
  using Scalar = double;
...
// TEST 7: PerOpMapRealSpace -- mirror of TEST 3 but in real space / tau,
// using the same endpoint-relative orbital mapping as production.
    ...
    for (int r = 0; r < nr; ++r)
      for (int b0 = 0; b0 < nb; ++b0)
        for (int b1 = 0; b1 < nb; ++b1) {
          ...
          const int r_new = RCluster::subtract(r0_new, r1_new);
