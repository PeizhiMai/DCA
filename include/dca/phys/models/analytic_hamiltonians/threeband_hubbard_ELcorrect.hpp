...
void ThreebandHubbard<PointGroupType>::initializeHInteraction(
  ...
  const double V_pd = parameters.get_V_pd();  // NN interaction between d and p bands
  const double V_pp = parameters.get_V_pp();  // NN interaction between p bands
  ...
  for (int s1 = 0; s1 < 2; s1++) {
    for (int s2 = 0; s2 < 2; s2++) {
        ...
        H_interaction(0, s1, 1, s2, origin) = V_pd;
        H_interaction(0, s1, 2, s2, origin) = V_pd;
        H_interaction(0, s1, 1, s2, 2) = V_pd;
        H_interaction(0, s1, 2, s2, 1) = V_pd;
        H_interaction(1, s1, 0, s2, origin) = V_pd;
        H_interaction(2, s1, 0, s2, origin) = V_pd;
        H_interaction(1, s1, 0, s2, 2) = V_pd;
        H_interaction(2, s1, 0, s2, 1) = V_pd;
  ...
  for (int s1 = 0; s1 < 2; s1++) {
    for (int s2 = 0; s2 < 2; s2++) {
        ...
        H_interaction(1, s1, 2, s2, origin) = V_pp;
        H_interaction(1, s1, 2, s2, 1) = V_pp;
        H_interaction(1, s1, 2, s2, 2) = V_pp;
        H_interaction(1, s1, 2, s2, 3) = V_pp;
        H_interaction(2, s1, 1, s2, origin) = V_pp;
        H_interaction(2, s1, 1, s2, 1) = V_pp;
        H_interaction(2, s1, 1, s2, 2) = V_pp;
        H_interaction(2, s1, 1, s2, 3) = V_pp;
...
void ThreebandHubbard<PointGroupType>::initializeH0WithQ(
  ...
  const auto U_dd = parameters.get_U_dd();  // interaction in d band
  const auto U_pp = parameters.get_U_pp();  // interaction in p bands
  const auto V_pd = parameters.get_V_pd();  // NN interaction between d and p bands
  const auto V_pp = parameters.get_V_pp();  // NN interaction between p bands
  ...
  for (int k_ind = 0; k_ind < KDmn::dmn_size(); ++k_ind) {
    ...
    for (int s = 0; s < 2; s++) {
      ...
      H_0(0, s, 0, s, k_ind) = -ep_d - U_dd - 8.0*V_pd + U_dd/2.0 + 4.0*V_pd;
      H_0(1, s, 1, s, k_ind) = -ep_p - U_pp - 4.0*V_pd - 8.0*V_pp + U_pp/2.0 + 4.0*V_pp + 2.0*V_pd;
      H_0(2, s, 2, s, k_ind) = -ep_p - U_pp - 4.0*V_pd - 8.0*V_pp + U_pp/2.0 + 4.0*V_pp + 2.0*V_pd;
